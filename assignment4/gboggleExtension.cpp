...
static void drawCube(int row, int col, char ch, bool invert,double theta,int arrowSize);
...
static void drawArrow(double x,double y,int size,bool q,double theta);
...
void labelCube(int row, int col, char letter) {
   ...
   drawCube(row, col, letter, false,0,0);
}
...
void highlightCube(int row, int col, bool flag,double theta,int arrowSize) {
   if (row < 0 || row >= gState.numRows || col < 0 || col >= gState.numCols) {
      error("labelCube called with invalid row, col arguments.");
   }
   drawCube(row, col, gState.letters[row][col], flag,theta,arrowSize);
}
...
static void drawEmptyCubes() {
   for (int row = 0; row < gState.numRows; row++)
      for (int col = 0; col < gState.numCols; col++)
          drawCube(row, col, ' ', false,0,0);   // erase all cubes, start with blank chars
}
...
static void drawCube(int row, int col, char ch, bool invert,double theta,int arrowSize) {
   ...
   drawArrow(cubeX(col) + gState.cubeSize/2.0,
             cubeY(row) + gState.cubeSize/2.0,arrowSize,invert,theta);
}
/*
This function paints line with the size and theta, which was given from draw cube function.
*/
static void drawArrow(double cx,double cy, int arrowSize,bool q,double theta){
	if(q){
		gwp->drawPolarLine(cx,cy,arrowSize,theta);
