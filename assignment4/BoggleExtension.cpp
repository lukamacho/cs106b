...
/*
This function return 5*5 grid and fills it with random chars from big boggle array.
*/
Grid<char> paintBigCubes(){
	Vector <string> vec;
	for(int i=0;i<25;i++){
...
/*This function allows you to choose the size of the board*/
int chooseSize(){
	int size=0;
	while(true){
		int  a=getInteger("Enter board size(4 or 5):");
		if(a==4){
			size=a;
			break;
		}else if(a==5){
...
void highlightWord(Vector<int> coordinates,Grid<char>& board,bool q){
	for(int i=0;i<coordinates.size();i+=2){
		...
		if(i!=coordinates.size()-2){
			size=24;
			/*Definition of the angle of the arrow*/
			if(coordinates[i]==coordinates[i+2]&&coordinates[i+1]>coordinates[i+3]){
			theta=-180.0;
			}else if(coordinates[i]==coordinates[i+2]&&coordinates[i+1]<coordinates[i+3]){
			theta=0.0;
			}else if(coordinates[i]<coordinates[i+2]&&coordinates[i+1]<coordinates[i+3]){
				theta=-45.0;
				size+=10;
		...
		highlightCube(coordinates[i], coordinates[i+1],q,theta,size);
			}
}
/*
